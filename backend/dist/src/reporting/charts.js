export function renderLineChartSvg(params) {
    ...
    return `
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-label="${escapeAttr(params.title)}">
  <rect x="0" y="0" width="${width}" height="${height}" fill="#ffffff" />
  <text x="${pad}" y="${pad - 6}" font-size="12" fill="#111" font-weight="600">${escapeText(params.title)}</text>
  <text x="${width - pad}" y="${pad - 6}" font-size="10" fill="#666" text-anchor="end">${escapeText(params.unit)}</text>
