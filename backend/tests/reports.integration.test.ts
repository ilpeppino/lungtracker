...
jest.mock('../../../src/auth/supabase.js', () => ({
...
const mockSupabaseStorageFrom = supabaseAdmin.storage.from as jest.MockedFunction<typeof supabaseAdmin.storage.from>;
const mockFetchReportData = fetchReportData as jest.MockedFunction<typeof fetchReportData>;
const mockHtmlToPdfBuffer = htmlToPdfBuffer as jest.MockedFunction<typeof htmlToPdfBuffer>;
const mockRenderReportHtml = renderReportHtml as jest.MockedFunction<typeof renderReportHtml>;
const mockSendReportLinkEmail = sendReportLinkEmail as jest.MockedFunction<typeof sendReportLinkEmail>;
const mockRandomToken = randomToken as jest.MockedFunction<typeof randomToken>;
...
describe('Reports API Integration Tests', () => {
  ...
  describe('Complete Report Generation Flow', () => {
    it('should successfully generate and email a complete health report', async () => {
      ...
      global.crypto = {
        ...global.crypto,
        randomUUID: jest.fn(() => testReportId as `${string}-${string}-${string}-${string}-${string}`)
      };
    ...
    it('should handle report listing and management', async () => {
      ...
      expect(listResponse.body).toEqual(mockReports);
      expect(mockLimit).toHaveBeenCalledWith(10);

      // Test report revocation
